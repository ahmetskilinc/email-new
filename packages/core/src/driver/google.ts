...
export class GoogleMailManager implements MailManager {
  ...
  constructor(public config: ManagerConfig) {
    this.auth = new OAuth2Client(this.config.clientId!, this.config.clientSecret!)
    ...
    return this.withErrorHandler(
      ...
      async () => {
        ...
        const processLabelEffect = (label: any) =>
          Effect.tryPromise({
            try: () =>
              ...
              _tag: "LabelFetchFailed" as const,
              error,
              labelId: label.id,
            }),
          }).pipe(
            Effect.map((res) => {
              ...
              const isTotalLabel =
                labelName === "drafts" || labelName === "sent"
              return {
                label: labelName,
                count: Number(
                  isTotalLabel ? res.data.threadsTotal : res.data.threadsUnread
                ),
              }
            })
          )
        ...
        const mainEffect = Effect.gen(function* () {
          ...
          if (
            "_tag" in userLabelsResult &&
            userLabelsResult._tag === "LabelListFailed"
          ) {
            return []
          }
  ...
  private getQuotaUser() {
    return this.config.auth?.email
      ? `${this.config.auth.email}-${process.env.NODE_ENV ?? 'development'}`
      : undefined
  }
  public list(params: {
    folder: string
    ...
    return this.withErrorHandler(
      "list",
      async () => {
        const { folder: normalizedFolder, q: normalizedQ } =
          this.normalizeSearch(folder, q ?? "")
          ...
          res.data.messages.map(async (message) => {
            const bodyData =
              ...
              (message.payload?.parts
                ? findHtmlBody(message.payload.parts)
                : "") ||
      ...
      async () => {
        ...
        if (data.cc)
          msg.setCc(
            data.cc
              ?.split(", ")
              .map((recipient: string) => ({ addr: recipient }))
          )
        if (data.bcc)
          msg.setBcc(
            data.bcc
          ...
          for (const attachment of data.attachments) {
            ...
            if (typeof (attachment as any)?.base64 === "string") {
              ...
              const buffer = Buffer.from(
                await (attachment as any).arrayBuffer()
              )
  ...
  > {
    ...
    const subject =
      payload?.headers?.find((h) => h.name?.toLowerCase() === "subject")
        ?.value || ""
    const references =
      payload?.headers?.find((h) => h.name?.toLowerCase() === "references")
        ?.value || ""
    const inReplyTo =
      payload?.headers?.find((h) => h.name?.toLowerCase() === "in-reply-to")
        ?.value || ""
    const messageId =
    ...
    const replyTo =
      payload?.headers?.find((h) => h.name?.toLowerCase() === "reply-to")
        ?.value || undefined
    const toHeaders =
      payload?.headers
  ...
  private async withErrorHandler<T>(
    ...
    try {
      ...
      if (isFatal) await this.config.onAuthFailure?.(this.config.auth.userId)
  ...
  private withSyncErrorHandler<T>(
    ...
    try {
      ...
      if (isFatal) void this.config.onAuthFailure?.(this.config.auth.userId)
      throw new StandardizedError(error, operation, context)
    }
  }
  ...
  private async resolveLabelId(labelName: string): Promise<string> {
    ...
    if (!created || !created.id) {
      throw new Error(
        `Failed to create or retrieve Gmail label '${labelName}'.`
      )
    }
