...
/**
 * iCloud Mail over IMAP/SMTP with an app-specific password.
 *
 * Apple's officially supported path for third-party clients, and the fallback
 * whenever the `mailws` web-service provider is unavailable, unauthenticated,
 * or asked for something Apple's private protocol does not cover (sending).
 */
export class ICloudImapMailManager implements MailManager {
