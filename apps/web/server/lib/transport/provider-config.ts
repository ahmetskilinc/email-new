// @ts-nocheck
import { ICLOUD_IMAP_DEFAULTS } from "./icloud-imap-defaults"
...
export const ICLOUD_CONFIG: ImapProviderConfig = ICLOUD_IMAP_DEFAULTS
...
export function buildLabelToFolder(
...
function matchFolderByName(
  ...
  for (const mb of mailboxes) {
    if (
      lowered.includes(mb.path.toLowerCase()) ||
      lowered.includes(mb.name.toLowerCase())
    ) {
      return mb.path
    }
  }
...
export async function autoDiscoverFolders(
  ...
  try {
    ...
    if (!folders.drafts) {
      folders.drafts =
        matchFolderByName(mailboxes, ["Drafts", "Draft", "INBOX.Drafts"]) ??
        "Drafts"
    }
    if (!folders.trash) {
      folders.trash =
